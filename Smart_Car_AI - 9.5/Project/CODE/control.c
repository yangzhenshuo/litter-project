...
static float ChangeIA = 120; //变积分控制A系数
...
void GetMotorSpeed(void)
{
    ...
    CarInfo.RunDistance2 += (CarPulse.L1 - CarPulse.R1 - CarPulse.L2 + CarPulse.R2 ) / 4; //更新小车y轴运行距离
		coordinate.x = (CarInfo.RunDistance2 / 220)  *5.5 * PI  + orignal_pos.x * 200;
		coordinate.y = (CarInfo.RunDistance1 / 220)  *5.5 * PI  + orignal_pos.y * 200;
}
/***********************************************************
 * @brief 速度变积分函数
 * @param delta 偏差
...
void PositionControl(void)
{
    float PositionDelta_x; //位置偏差
        PositionDelta_x = position.x * 200 - (CarInfo.RunDistance1 / 220) * 5.5 * PI; //计算偏差量
    PositionDeviationIntegrate_x += PositionDelta_x;
    if (PositionDeviationIntegrate_x > integral_error)       // 100//200
        PositionDeviationIntegrate_x = integral_error;       // 100//200
    else if (PositionDeviationIntegrate_x < -integral_error) // 100//200
        PositionDeviationIntegrate_x = -integral_error;      // 100//2000
...
//		 CorrectDeviationIntegrate_x = -10;      // 100//2000
    ...
    if (CorrectControlOut_x > 50)
    {
        CorrectControlOut_x = 50;
    }
    else if (CorrectControlOut_x < -50)
    {
       CorrectControlOut_x = -50;
    }
...
//        CorrectDeviationIntegrate_y = -10;      // 100//2000
    ...
    if (CorrectControlOut_y > 50)
    {
        CorrectControlOut_y = 50;
    }
    else if (CorrectControlOut_y < -50)
    {
       CorrectControlOut_y = -50;
    }

...
void motor_init(void)
{
	...
	pwm_init(f_PWM_L, 15000, 0);												// 
	pwm_init(f_PWM_R, 15000, 0);												// 
	pwm_init(r_PWM_L, 15000, 0);												// 
	pwm_init(r_PWM_R, 15000, 0);												// 初始化频率17KHz 占空比初始为0
}
/***********************************************************
 * @brief 电机转速,方向设置
...
void motor_control(int32 f_duty_l, int32 f_duty_r, int32 r_duty_l, int32 r_duty_r)//从前至后依次为右前轮，左前轮，右后轮，左后轮
{
    ...
    if(f_duty_l >= 0)											
    {
        gpio_set(f_DIR_L, GPIO_LOW);							// DIR输出高电平
        pwm_duty(f_PWM_L, f_duty_l);						    // 计算占空比
    }
    else													
    {
        gpio_set(f_DIR_L, GPIO_HIGH);							// DIR输出低电平
        pwm_duty(f_PWM_L, -f_duty_l);							// 计算占空比
    }
    
    if(f_duty_r >= 0)											
    {
        gpio_set(f_DIR_R, GPIO_LOW);							// DIR输出高电平
        pwm_duty(f_PWM_R, f_duty_r);							// 计算占空比
    }
    else												
    {
        gpio_set(f_DIR_R, GPIO_HIGH);							// DIR输出低电平
        pwm_duty(f_PWM_R, -f_duty_r);							// 计算占空比
    }
}
/***********************************************************

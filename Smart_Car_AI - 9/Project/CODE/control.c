...
#define MOTOR_PWM_MAX (500.0 * 60) //电机最大PWM
#define Speed_max 80
#define error 10
...
static float ChangeIA = 2*Speed_max; //变积分控制A系数
...
/*************矫正控制参数******************/
static float CorrectControlOut_x = 0;         // x轴位置输出量
static float CorrectDeviationIntegrate_x = 0; // x轴累计位置差
static float CorrectDeltaPrev_x = 0;          // x轴前一次位置偏差
static float CorrectControlOut_y = 0;         // y轴位置输出量
static float CorrectDeviationIntegrate_y = 0; // y轴累计位置差
static float CorrectDeltaPrev_y = 0;          // y轴前一次位置偏差
/*************车体状态参数******************/
...
void ResetControlArgs(void)
{
    ...
    PositionDeltaPrev_y = 0;          // y轴前一次位置偏差
		/*************矫正控制参数******************/
		CorrectControlOut_x = 0;         // x轴位置输出量
		CorrectDeviationIntegrate_x = 0; // x轴累计位置差
		CorrectDeltaPrev_x = 0;          // x轴前一次位置偏差
		CorrectControlOut_y = 0;         // y轴位置输出量
		CorrectDeviationIntegrate_y = 0; // y轴累计位置差
		CorrectDeltaPrev_y = 0;          // y轴前一次位置偏差
    /********左前轮********/
     SpeedSet_L1 = 0;              //左前轮设置速度
     f_LeftSpeedDeltaPrev = 0;     //左前轮前一次速度偏差
     f_LeftSpeedDeltaPrevPrev = 0; //左前轮前前一次速度偏差

     f_LeftSpeedControlBais = 0; //左前轮速度控制增量
     f_LeftSpeedControlOut = 0;  //左前轮速度环输出

    /********左后轮********/
     SpeedSet_L2 = 0;              //左后轮设置速度
     r_LeftSpeedDeltaPrev = 0;     //左后轮前一次速度偏差
     r_LeftSpeedDeltaPrevPrev = 0; //左后轮前前一次速度偏差

     r_LeftSpeedControlBais = 0; //左后轮速度控制增量
    r_LeftSpeedControlOut = 0;  //左后轮速度环输出

    /********右前轮********/
     SpeedSet_R1 = 0;               //右前轮设置速度
     f_RightSpeedDeltaPrev = 0;     //右前轮前一次速度偏差
    f_RightSpeedDeltaPrevPrev = 0; //右前轮前前一次速度偏

    f_RightSpeedControlBais = 0; //右前轮速度控制增量
     f_RightSpeedControlOut = 0;  //右前轮速度环输出

    /********右后轮********/
    SpeedSet_R2 = 0;               //右后轮设置速度
    r_RightSpeedDeltaPrev = 0;     //右后轮前一次速度偏差
    r_RightSpeedDeltaPrevPrev = 0; //右后轮前前一次速度偏

    r_RightSpeedControlBais = 0; //右后轮速度控制增量
    r_RightSpeedControlOut = 0;  //右后轮速度环输出
		/*************车体状态参数******************/
    Xspeed=0;
    Yspeed=0;
}
...
void GetMotorSpeed(void)
{
    ...
    coefficient(position);						                            //获取系数
    Yspeed= (CarPulse.L1 + CarPulse.R1 + CarPulse.L2 + CarPulse.R2 ) / 4; //更新小车x轴运行距离
    Xspeed= (CarPulse.L1 - CarPulse.R1 - CarPulse.L2 + CarPulse.R2 ) / 4; //更新小车y轴运行距离
	  CarInfo.RunDistance1 += Posvalue_y*(Yspeed + LastYspeed)/2;    //小车运行距离
...
{
    float PositionDelta_x; //y轴位置偏差
        PositionDelta_x = position.x * 200 - CarInfo.RunDistance1; //计算偏差量
    ...
    float PositionDelta_y; //x轴位置偏差
    PositionDelta_y = position.y * 200 - CarInfo.RunDistance2; //计算偏差量

...
//		SystemSettings.Is_control = 'T';			
		...
		{
			if(DetAbs(coordinate.x, CarInfo.goal_x * 200) < 10 && DetAbs(CarInfo.goal_y * 200, coordinate.y) < 10) //判断到达点位置
			{
...
void visual_pursit(void *parameter)
{
	...
	while(1)
	{
		...
		{
			CarInfo.distance2 = barycentre[0] - 80;                    //x轴偏差
			CarInfo.distance1 = 60 - barycentre[1];                     //y轴偏差	
		}
...
static inline int collision(int e, coordinateTypedef pos)
{
	...
	for(int i =0; i < 20; i++)//判断是否有点在运行路径上
	{

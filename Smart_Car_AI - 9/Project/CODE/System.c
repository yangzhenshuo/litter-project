...
//ControlPidTypedef SpeedControlPid = {150, 50, 90};{8, 0, 12};    //速度控制pid参数初始化
ControlPidTypedef SpeedControlPid = {200,100,200};      //速度控制pid参数初始化
ControlPidTypedef AngleControlPid = {10, 0, 12};         //角度pid参数
ControlPidTypedef PositionPid_y = {0.7, 0, 3};          //y轴位置pid参数
ControlPidTypedef PositionPid_x = {0.7, 0, 3};          //x轴位置pid参数
ControlPidTypedef CorrectControlPid={0.3, 0, 1};        //矫正pid参数
...
coordinateTypedef orignal_pos = {0, 0};                 //车绝对位置值
coordinateTypedef coordinate = {0, 0};                  //编码器计算得到车位置（用于到达判断）
...
{
	SystemSettings.Is_control = 'T';              //控制 关
  ...
  SystemSettings.ChangeIEnable = 'F';           //速度变积分控制 开

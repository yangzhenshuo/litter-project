...
{
  char temp[100];
  sprintf(temp, "(%d,%d),(%d,%d);\n",(int)CarInfo.goal_x, (int)CarInfo.goal_y, (int)barycentre[0], (int)barycentre[1]);
  seekfree_wireless_send_buff((uint8 *)temp, strlen(temp));
}
/***********************************************************
 * @brief 状态
...
{
  char temp[100];
  sprintf(temp, "%c,%c,%c,(%d,%d);\n", SystemSettings.IsFound_dot, SystemSettings.Is_control, SystemSettings.IsCarry, (int)position.x, (int)position.y);
  seekfree_wireless_send_buff((uint8 *)temp, strlen(temp));
}
/***********************************************************
 * @brief 目标数据
...
{
  char temp[100];
  sprintf(temp, "(%d,%d),(%d,%d);\n", (int)CarInfo.goal_x, (int)CarInfo.goal_y, (int)coordinate.x, (int)coordinate.y);
  seekfree_wireless_send_buff((uint8 *)temp, strlen(temp));
}
/***********************************************************
 * @brief 地图点的数据
...
void Report(void)
{
  ...
  seekfree_wireless_send_buff((uint8 *)temp, strlen(temp));
	sprintf(temp, "(%d,%d),(%d,%d);\n",(int)CarInfo.goal_x, (int)CarInfo.goal_y, (int)CarInfo.RunDistance1, (int)CarInfo.yaw);
  seekfree_wireless_send_buff((uint8 *)temp, strlen(temp));

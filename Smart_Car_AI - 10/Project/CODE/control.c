...
#define Pos_value 220
#define correct_max 70
#define error 10
...
void GetMotorSpeed(void)
{
    ...
    CarInfo.RunDistance2 += (CarPulse.L1 - CarPulse.R1 - CarPulse.L2 + CarPulse.R2 ) / 4; //更新小车y轴运行距离
		coordinate.x = (CarInfo.RunDistance2 / Pos_value)  *5.5 * PI  + orignal_pos.x * 200;
		coordinate.y = (CarInfo.RunDistance1 / Pos_value)  *5.5 * PI;
}
/***********************************************************
 * @brief 速度变积分函数
 * @param delta 偏差
...
void PositionControl(void)
{
    float PositionDelta_x; //位置偏差
        PositionDelta_x = position.x * 200 - (CarInfo.RunDistance1 / Pos_value) * 5.5 * PI; //计算偏差量
    ...
    PositionDelta_y = position.y * 200 - (CarInfo.RunDistance2 / Pos_value) * 5.5 * PI; //计算偏差量
...
//		 CorrectDeviationIntegrate_x = -10;      // 100//2000
    ...
    if (CorrectControlOut_x > correct_max)
    {
        CorrectControlOut_x = correct_max;
    }
    else if (CorrectControlOut_x < -correct_max)
    {
       CorrectControlOut_x = -correct_max;
    }
...
//        CorrectDeviationIntegrate_y = -10;      // 100//2000
    ...
    if (CorrectControlOut_y > correct_max)
    {
        CorrectControlOut_y = correct_max;
    }
    else if (CorrectControlOut_y < -correct_max)
    {
       CorrectControlOut_y = -correct_max;
    }

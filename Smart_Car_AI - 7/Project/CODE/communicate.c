...
{
  char temp[100];
  sprintf(temp, "%c,%c,(%d,%d);\n", SystemSettings.IsFound_dot, SystemSettings.Is_control, (int)CarInfo.dotnum, (int)CarInfo.num);
...
{
  char temp[100];
  sprintf(temp, "(%d,%d),(%d,%d);\n",(int)CarInfo.goal_x, (int)CarInfo.goal_y, trackpoint[0], trackpoint[1]);
  seekfree_wireless_send_buff((uint8 *)temp, strlen(temp));
}
/***********************************************************
 * @brief 目标数据

...
char command_recv[100]; //储存接收到的信息
char byte_recv;         //储存接收到的字节
 ...
 * @return
 ***********************************************************/
void handle_message(char *CommandRecv, uint8 CommandLength)
{
  char *p = CommandRecv;
...
void wireless_uart_callback(LPUART_Type *base, lpuart_handle_t *handle, status_t status, void *userData)
{
  if (kStatus_LPUART_RxIdle == status)
  {
    ...
    byte_recv = (char)wireless_rx_buffer;
    command_recv[command_length++] = byte_recv;
    if (byte_recv == ';')
    {
      handle_message(command_recv, command_length);
      command_length = 0;
    }
  }

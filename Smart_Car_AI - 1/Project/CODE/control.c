...
void GetMotorSpeed(void)
{
    ...
    CarInfo.RunDistance1 += CarInfo.SpeedSet_x/10; //更新小车运行距离
	  CarInfo.RunDistance2 += CarInfo.SpeedSet_y/10; //更新小车运行距离
...
static inline void AngleControlCal(void)
{
    ...
    AngleDeltaPrev = AngleDelta- AngleDeltaPrev;             //更新为上次偏差 
...
{
    float PositonDelta_x;                                       //位置偏差
    PositonDelta_x = position.x - CarInfo.RunDistance1; //计算偏差量
	  PositionDeviationIntegrate_x += PositonDelta_x;
    ...
    CarInfo.delet1= CarInfo.RunDistance2;
		float PositonDelta_y;                                       //位置偏差
    PositonDelta_y = position.y - CarInfo.RunDistance2; //计算偏差量
	  PositionDeviationIntegrate_y += PositonDelta_y;

...
static inline void CarPostureCal(void)
{
  ...
  icm.gyro_x = ((float)icm_gyro_x - GyroOffset.x) / 16.4;
  icm.gyro_y = ((float)icm_gyro_y - GyroOffset.y) / 16.4;
  icm.gyro_z = ((float)icm_gyro_z - GyroOffset.z) / 16.4;
  CarInfo.delet2 = icm_gyro_x - GyroOffset.x;
...
void GetMotorSpeed(void)
{
  ...
  CarInfo.RunDistance += (CarPulse.L1 + CarPulse.L2 + CarPulse.R1 + CarPulse.R2) / 40; //更新小车运行距离
...
//{
//     float PositonDelta_x;                                       //位置偏差
//     PositonDelta_x = position.x - angle_dot[0]; //计算偏差量
...
//	  float PositonDelta_y;                                       //位置偏差
//     PositonDelta_y = position.y - angle_dot[1]; //计算偏差量
...
{
  float PositonDelta_x;                              //位置偏差
  PositonDelta_x = position.x - CarInfo.RunDistance; //计算偏差量
  PositionDeviationIntegrate_x += PositonDelta_x;
  ...
  CarInfo.SpeedSet_x = PositionControlOut_x;
  CarInfo.delet1 = CarInfo.RunDistance;
}